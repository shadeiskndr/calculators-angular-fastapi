...
class Numbers(BaseModel):
    ...
    method: VaRMethod = Field(
        ...
        description="Method to use for VaR calculation"
    )
    ...
    def validate_data_quality(cls, values):
        ...
        if len(numbers) > 0:
            ...
            if len(set(numbers)) == 1:
                raise ValueError('All values are identical - cannot calculate meaningful VaR')
            ...
            if iqr > 0:
                ...
                outliers = np.sum((arr < (q25 - outlier_threshold)) | (arr > (q75 + outlier_threshold)))
                if outliers > len(numbers) * 0.1:  # More than 10% outliers
                    logger.warning(f"Dataset contains {outliers} potential outliers ({outliers/len(numbers)*100:.1f}%)")
...
class VaRResponse(BaseModel):
    ...
    additional_stats: Optional[dict] = Field(None, description="Additional statistical information")
...
def calculate_var(data: Numbers):
    ...
    """
    try:
        logger.info(f"Calculating VaR with {len(data.numbers)} data points, "
